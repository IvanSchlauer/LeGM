...
public interface PlayerTeamRepository extends JpaRepository<PlayerTeam, Long> {
    ...
    @Query("SELECT pt FROM PlayerTeam pt WHERE pt.teamID = :teamID")
    public List<GamePlayer> getPlayerByTeam(Long teamID);

    @Query("SELECT p FROM PlayerTeam pt INNER JOIN Player p ON pt.playerID=p.playerId WHERE pt.startDate <= :date AND pt.endDate >= :date OR pt.endDate = NULL AND pt.teamID = :teamID")
